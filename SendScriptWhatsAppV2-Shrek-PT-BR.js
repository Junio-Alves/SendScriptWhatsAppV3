async function enviarScript(scriptText) {
    ...
    if (!main) throw new Error("A conversa não está aberta!");

    const textarea = main.querySelector(`div[contenteditable="true"]`);
    if (!textarea) throw new Error("Não há campo de texto ativo!");
    ...
    for (const line of lines) {
        ...
        // Tenta encontrar o botão de envio atualizado
        const botaoEnviar =
            main.querySelector('button[aria-label="Send"]') ||
            main.querySelector('[data-icon="wds-ic-send-filled"]')?.closest('button');

        if (botaoEnviar) {
            botaoEnviar.click();
        } else {
            console.warn("❗ Botão de enviar não encontrado. Pulei essa mensagem.");
        }
